...
public class Receiver implements Runnable {
	private ArrayBlockingQueue<Packet> DataQueue = new ArrayBlockingQueue<Packet>(LinkLayer.QueueCap);
	...
	private void SIFS() {
		...
		while (true) {
			if (theRF.clock() + LinkLayer.clockModifier % 100 != 0
					|| theRF.clock() + LinkLayer.clockModifier % 100 != 50) {
	...
	public Receiver(RF theRF, Boolean[] ackQueue, ArrayBlockingQueue<Packet> dataQueue, short ourMAC,
			PrintWriter output) {
	...
	public void run() {
		...
		while (true) { // loop to keep thread alive
			...
			if (DataQueue.size() >= LinkLayer.QueueCap) {
				System.out.print("Dropping Packet Queue Full");
				byte[] IncomingByteArray = theRF.receive();
				IncomingByteArray = null;
			}
			...
			if (toUs(IncomingByteArray) == ourMAC || toUs(IncomingByteArray) == -1) { // if the packet was meant for us
				if (IncomingPacket.getFrameType() == ACK) { // if packet received was an ack
					...
					if (IncomingPacket.getFrameType() == 0b010) { // if its a beacon
						...
						long ourTime = theRF.clock() + LinkLayer.clockModifier;
						...
						// }
						//
						long depacket = theRF.clock();
						long theirTime = 0;
						for (int i = 0; i < data.length; i++) {
							theirTime = (theirTime << 8) + (data[i] & 0xff);
						}
						depacket = depacket - theRF.clock();

						theirTime = theirTime + depacket;
