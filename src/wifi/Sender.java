...
public class Sender implements Runnable{
	...
	byte[] data;
	int backoffWindow;
	int timeoutWindow = 3500; //4 seconds seems to be the magic first number
    ...
    private void StartBackoff(int backoff) { //handles the waiting till transmission is open
    	...
    	while(sent==false) {
    		...
    		if(theRF.inUse()==false) {
    			...
    			else backOffWindow = backOffWindow * 2;//increase the back off Window
    		}
    	}
    }
			...
			// if(sendQueue.isEmpty()==true) {
			 ...
			 //else {
				 if(LinkLayer.beaconInterval!=-1) { //Beacon Path
					 ...
					 if(lastBeaconTimeStamp+(LinkLayer.beaconInterval*1000)<=modifiedTime) { //we have to send a beacon
						 createBeacon(modifiedTime);
						 BeaconsTurn=true;
						 lastBeaconTimeStamp = modifiedTime;
					 }
				 }
				if(!sendQueue.isEmpty()&&BeaconsTurn==false) {packet = sendQueue.poll();}
